...
contract TestBase {
    ...
    bytes32 internal constant ENVIRONMENT_ID = keccak256("curve-vyper-reentrancy-trap-hoodi");
...
contract ExploitReproductionTest is TestBase {
    ...
    function _deploy() internal returns (CurveVyperReentrancyTrap trap, CurveVyperReentrancyResponse response, CurveVyperReentrancyProtocolMock protocol, CurveVyperReentrancyAttacker attacker) {
        MockToken tokenImpl = new MockToken();
        vm.etch(TOKEN, address(tokenImpl).code);
        CurveVyperReentrancyProtocolMock protocolImpl = new CurveVyperReentrancyProtocolMock(TOKEN);
        vm.etch(TARGET, address(protocolImpl).code);
        protocol = CurveVyperReentrancyProtocolMock(TARGET);
        attacker = new CurveVyperReentrancyAttacker(TARGET);
        response = new CurveVyperReentrancyResponse(REGISTRY_ADDR);
        protocol.setToken(TOKEN);
        protocol.setEmergencyModule(address(response));
        protocol.seedHealthy(address(attacker));
        CurveVyperReentrancyEnvironmentRegistry registryImpl = new CurveVyperReentrancyEnvironmentRegistry(ENVIRONMENT_ID, TARGET, address(response), address(response), true);
        vm.etch(REGISTRY_ADDR, address(registryImpl).code);
        CurveVyperReentrancyEnvironmentRegistry(REGISTRY_ADDR).setConfig(ENVIRONMENT_ID, TARGET, address(response), address(response), true);
        trap = new CurveVyperReentrancyTrap();
    }

    function _samples(CurveVyperReentrancyTrap trap, bool exploit) internal returns (bytes[] memory data) {
        data = new bytes[](5);
        data[data.length - 1] = trap.collect();
        if (exploit) {
            CurveVyperReentrancyAttacker attacker = new CurveVyperReentrancyAttacker(TARGET);
    ...
    function testExploitSucceedsWithoutTrap() public {
        (, , CurveVyperReentrancyProtocolMock protocol, CurveVyperReentrancyAttacker attacker) = _deploy();
        attacker.stageExploit();
...
contract TrapLifecycleTest is TestBase {
    ...
    function testCollectDecodesHealthyState() public {
        (CurveVyperReentrancyTrap trap,,,) = _deploy();
    ...
    function testShouldRespondTrueOnExploitWindow() public {
        ...
        assertTrue(alert.invariantId == keccak256("CURVE_ACCOUNTED_BALANCE_MISMATCH_V2"), "invariant id");
...
contract ContainmentTest is TestBase {
    ...
    function testExploitContainedWithTrap() public {
        (CurveVyperReentrancyTrap trap, CurveVyperReentrancyResponse response, CurveVyperReentrancyProtocolMock protocol, CurveVyperReentrancyAttacker attacker) = _deploy();
        (bool ok, bytes memory payload) = trap.shouldRespond(_samples(trap, true));
...
contract ResponseAuthorizationTest is TestBase {
    ...
    function testOnlyDroseraCanCallResponse() public {
        (CurveVyperReentrancyTrap trap, CurveVyperReentrancyResponse response,,) = _deploy();
    ...
    function testResponseRejectsWrongEnvironment() public {
        (, CurveVyperReentrancyResponse response,,) = _deploy();
